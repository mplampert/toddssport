...
async function fetchCategoryProducts(
  ...
  try {
    ...
    console.log(`[${categorySlug}] Fetching from ProductInfo endpoint via proxy...`);

    const response = await proxyFetch(productInfoUrl, "GET");
...
async function fetchFromCustomBuilder(
  ...
  try {
    ...
    console.log(`[${categorySlug}] Trying Custom Builder API via proxy: ${cbApiUrl}`);

    const response = await proxyFetch(cbApiUrl, "GET");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const specificCategory = url.searchParams.get("category");
    ...
    for (const [slug, info] of Object.entries(categoriesToSync)) {
      ...
      results.push({ slug, categoryId: info.id, categoryName: info.name, status, productsFound: products.length, error });
    ...
    return new Response(
      JSON.stringify({ success: true, summary, categories: results }),

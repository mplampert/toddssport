...
import { Loader2, Check, X, Palette, Save, ChevronDown, ChevronUp, DollarSign } from "lucide-react";
...
interface Props {
  item: {
    ...
    style_id: number;
    allowed_colors: any;
    size_upcharges?: Record<string, number> | null;
...
const UPCHARGE_PRESETS: { label: string; sizes: Record<string, number> }[] = [
  { label: "+$2 for 2XL, +$3 for 3XL", sizes: { "2XL": 2, "3XL": 3 } },
  { label: "+$2 for 2XL, +$3 for 3XL, +$4 for 4XL+", sizes: { "2XL": 2, "3XL": 3, "4XL": 4, "5XL": 5, "6XL": 6 } },
  { label: "Clear all upcharges", sizes: {} },
];
...
export function ProductVariantsTab({ item, storeId }: Props) {
  ...
  const [sizeUpcharges, setSizeUpcharges] = useState<Record<string, number>>(
    (item.size_upcharges as Record<string, number>) ?? {}
  );
  ...
  const allSizes = useMemo(() => {
    ...
    colorOptions.forEach((c) => c.sizes.forEach((s) => set.add(s.name)));
    return [...set].sort((a, b) => {
  ...
  const applyUpchargePreset = (preset: Record<string, number>) => {
    setSizeUpcharges(preset);
    setDirty(true);
  };

  const updateUpcharge = (sizeName: string, value: string) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      // Clean upcharges: remove zero/empty entries
      const cleanUpcharges: Record<string, number> = {};
      for (const [k, v] of Object.entries(sizeUpcharges)) {
        if (v > 0) cleanUpcharges[k] = v;
      }
      const { error } = await supabase
        .from("team_store_products")
        .update({
          allowed_colors: payload as any,
          size_upcharges: Object.keys(cleanUpcharges).length > 0 ? cleanUpcharges : null,
        } as any)
        .eq("id", item.id);
  ...
  const hasAnyUpcharges = Object.values(sizeUpcharges).some((v) => v > 0);
  ...
  return (
    <div className="space-y-6">
      {/* ── Size Upcharges Section ── */}
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <DollarSign className="w-4 h-4 text-muted-foreground" />
          <h3 className="text-sm font-semibold text-foreground">Size Upcharges</h3>
          {hasAnyUpcharges && (
            <Badge variant="secondary" className="text-[9px]">
              {Object.keys(sizeUpcharges).filter((k) => sizeUpcharges[k] > 0).length} sizes with upcharge
            </Badge>
          )}
        </div>
        <p className="text-xs text-muted-foreground">
          Add price adjustments for extended sizes (e.g., 2XL, 3XL). These are added to the base price on the storefront.
        </p>
        {/* Preset buttons */}
        <div className="flex flex-wrap gap-1.5">
          {UPCHARGE_PRESETS.map((preset, i) => (
            <Button
              key={i}
              size="sm"
              variant="outline"
              className="text-[10px] h-7"
              onClick={() => applyUpchargePreset(preset.sizes)}
            >
              {preset.label}
            </Button>
          ))}
        </div>
        {/* Per-size upcharge grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {allSizes.map((sizeName) => (
            <div key={sizeName} className="flex items-center gap-2 bg-muted/30 rounded-md px-2 py-1.5">
              <span className="text-xs font-medium w-10 shrink-0">{sizeName}</span>
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  value={sizeUpcharges[sizeName] ?? ""}
                  onChange={(e) => updateUpcharge(sizeName, e.target.value)}
                  placeholder="0.00"
                />
              </div>
            </div>
                    ...
                    {selected && (
                      <>
                        ...
                        <div className="grid grid-cols-1 gap-0.5">
                          {/* Header */}
                          <div className="grid grid-cols-[24px_1fr_80px_60px_60px] gap-x-3 text-[10px] font-medium text-muted-foreground uppercase tracking-wider pb-1">
                            <span />
                            <span>Size</span>
                            <span>SKU</span>
                            <span className="text-right">Price</span>
                            <span className="text-right">Upcharge</span>
                          </div>
                          {sortedSizes.map((s) => {
                            const sizeEnabled = isSizeEnabled(c.code, s.name);
                            const upcharge = sizeUpcharges[s.name] ?? 0;
                            return (
                              <div
                                key={s.sku || s.name}
                                className={`grid grid-cols-[24px_1fr_80px_60px_60px] gap-x-3 items-center py-1 rounded px-0.5 ${
                              ...
                              >
                                ...
                                <span className="text-xs text-right tabular-nums">
                                  {upcharge > 0 ? (
                                    <span className="text-amber-600">+${upcharge.toFixed(2)}</span>
                                  ) : "—"}
                                </span>
                              </div>
                            );
